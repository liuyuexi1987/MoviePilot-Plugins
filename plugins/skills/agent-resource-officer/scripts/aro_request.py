...
GLOBAL_CONFIG_PATH_DISPLAY = "~/.config/agent-resource-officer/config"
GLOBAL_CONFIG_PATH = os.path.expanduser(GLOBAL_CONFIG_PATH_DISPLAY)
SKILL_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LOCAL_CONFIG_PATH = os.path.join(SKILL_DIR, "config")
...
def display_path(path):
    ...
    if value.startswith(home_dir + os.sep):
        return "~/" + os.path.relpath(value, home_dir)
    return value
...
def resolve_config_path(explicit_path=""):
    ...
    env_path = os.environ.get("ARO_CONFIG_PATH")
    if env_path:
        path = normalize_path(env_path)
        return {
            "path": path,
            "display": display_path(path),
            "source": "env:ARO_CONFIG_PATH",
            "exists": os.path.exists(path),
        }
    ...
    if os.path.exists(LOCAL_CONFIG_PATH):
        return {
            ...
            "display": display_path(LOCAL_CONFIG_PATH),
            "source": "skill:local_config",
            "exists": True,
        }

    return {
        "path": GLOBAL_CONFIG_PATH,
        "display": GLOBAL_CONFIG_PATH_DISPLAY,
        "source": "global:default_config",
        "exists": os.path.exists(GLOBAL_CONFIG_PATH),
    }


def read_config(config_meta=None):
    config = {}
    meta = dict(config_meta or resolve_config_path())
    config_path = str(meta.get("path") or "").strip()
...
def calibration_payload(include_repo_update=False):
    ...
    return {
        ...
        "agent_instruction": "用户说“校准影视技能”时，先检查 MoviePilot-Plugins 仓库更新；无论是否成功 pull，都必须重新读取本地 skill 文件并覆盖当前会话的资源流语义。",
        "hard_rules": [
...
def format_calibration_text(payload):
    ...
    lines.extend([
        "",
        "无论仓库是否成功拉取最新版，都必须继续重新读取本地 skill 文件并覆盖当前会话规则。",
        "",
...
def selftest_result():
    ...
    explicit_config_meta = resolve_config_path("/tmp/aro-helper-test.conf")
    check("config_path_arg_has_priority", explicit_config_meta.get("source") == "arg:--config" and explicit_config_meta.get("display") == "/tmp/aro-helper-test.conf")
    check("config_path_default_falls_back_to_known_scope", resolve_config_path().get("source") in {"skill:local_config", "global:default_config"})
...
def main():
    ...
    config_meta = resolve_config_path(args.config or "")
    config, config_meta = read_config(config_meta)
    ...
    if args.command == "config-check":
        result = {
            ...
            "config_path": config_meta.get("display") or "",
            "config_source": config_meta.get("source") or "",
            "config_file_exists": bool(config_meta.get("exists")),
            "local_config_path": display_path(LOCAL_CONFIG_PATH),
            "local_config_exists": os.path.exists(LOCAL_CONFIG_PATH),
            "global_config_path": GLOBAL_CONFIG_PATH_DISPLAY,
