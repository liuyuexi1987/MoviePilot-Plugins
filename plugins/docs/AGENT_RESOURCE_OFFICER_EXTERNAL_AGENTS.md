...
盘搜、影巢、下载、编号选择、翻页、详情、Cookie 修复，继续走 agent-resource-officer skill / helper。
...
- `ARO_BASE_URL` 指向哪套 MoviePilot，`下载 / MP搜索 / PT搜索` 就使用哪套 MoviePilot。
...
- `刷新影巢Cookie` / `刷新夸克Cookie` 读取的是智能体所在电脑浏览器里的登录态，再写回 `MoviePilot`；跨机器部署时尤其要注意这点。
- 用户明确发出新的带片名入口命令时，例如 `搜索低智商犯罪`、`搜索 低智商犯罪`、`MP搜索 <片名>`、`PT搜索 <片名>`、`下载 <片名>`、`转存 <片名>`，只能 route 原话。即使发现旧会话、活跃候选或 route 报错，也不能自动 `pick 1` / `选择 1`，更不能提交下载或转存。
- 盘搜续搜括号年份规则：当用户在已有影视候选上下文后说"盘搜再搜一下 / 用盘搜搜 / 云盘再找一下"等续问，如果上一轮识别名是"片名 (年份)"或"片名（年份）"，首轮盘搜命令只使用片名，不把括号年份拼入搜索词。当用户直接输入"盘搜搜索 片名 (年份)"时，也将括号年份视为影视识别格式，首轮 route 为"盘搜搜索 片名"。`aro_request.py route` 已自动处理 `盘搜搜索` 的括号年份剥离。只有用户明确说"只搜/按/带/2026 版"等强约束时，才保留年份进入盘搜搜索词。`MP搜索` / `PT搜索` / `影巢搜索` 不受此规则影响。
...
| 盘搜、影巢、编号选择、翻页、详情、Cookie 修复 | `agent-resource-officer skill / helper` |
| `MP搜索 / PT搜索 / 下载` 这类片名资源流 | 优先 `agent-resource-officer skill / helper` |
...
这条命令会让智能体先检查并拉取 `MoviePilot-Plugins` 仓库最新版，再重新加载影视助手的关键规则。不要在普通 `搜索` 前主动清会话，否则会破坏正常编号续接。
