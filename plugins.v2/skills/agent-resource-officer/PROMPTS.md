...
```text
如果用户命令明显是资源流，例如”盘搜搜索/影巢搜索/MP搜索/PT搜索/转存/夸克转存/115转存/下载/选择/详情/n/下一页/编号”，不要先调用 MCP、tool_search、curl 或 raw API 探测，直接使用 agent-resource-officer helper 的 route/pick。带”第 3 集 / E03 / 更新至”等集数线索的”搜索”也要直接按 MP/PT 处理，不要回退云盘。注意：标题级”转存/115转存/夸克转存 + 片名”已经取消，route 后只会返回引导用户先搜索的提示。`更新检查 <片名>` / `查更新 <片名>` / `检查 <片名>` 已并回搜索语义，路由时当作搜索处理。
```

```text
如果用户明确说“MP搜索 / MP 搜索 / PT搜索 / PT 搜索 + 片名”，必须原样调用 route，不要改写成“搜索/盘搜搜索/影巢搜索/智能搜索”。这是用户指定 MoviePilot 原生/PT 搜索，不是普通盘搜。
```
...
```text
使用 agent-resource-officer skill，执行“转存 / 115转存 / 夸克转存”后，如果插件返回“命令已取消”“暂无结果”“盘搜无结果，可继续试：影巢搜索 片名”“当前不会自动降级执行影巢转存”这类提示，必须原样转发。不要自己追加“要不要试夸克转存”“可以重新盘搜”“可能是缓存清空”这类分析，也不要把无结果改写成新的推荐动作。
```

```text
使用 agent-resource-officer skill，执行“转存 / 115转存 / 夸克转存”时，route 原话一次以后就停在这条链路上。即使命令已取消或结果为空，也不要再自动补跑“MP搜索 / PT搜索 / 搜索 / raw API / 第二次 route”去替用户找下载方案。转存命令的下一步只能是原样回插件提示；除非用户明确改口，否则不要替用户续跑别的链路。
```

## Cloud Search

```text
使用 agent-resource-officer skill，盘搜搜索“蜘蛛侠”或影巢搜索“蜘蛛侠”。这两条入口只在盘搜和影巢之间补查，不进入 MP/PT。
```

```text
使用 agent-resource-officer skill，执行“盘搜搜索 大君夫人”或“影巢搜索 大君夫人”时，必须直接调用 route 对应原话。不要先自己做网页搜索，也不要先把结果加工成“推荐资源/分析结论”。优先原样展示插件返回的 `盘搜结果` 和 `影巢结果` 两段、原始编号、原始链接和下一步提示。
```

```text
使用 agent-resource-officer skill，执行“盘搜搜索 大君夫人”或“影巢搜索 大君夫人”后，不要把返回改写成自己的表格摘要，不要把 115 和夸克各自重新从 1 开始编号，也不要只摘“亮点”。如果插件返回了全局编号和 `影巢结果` 段落，就原样保留；如果插件提示“影巢候选未自动展开”，也原样保留这句。
```
...
```text
使用 agent-resource-officer skill，当用户在已有影视候选上下文后说“盘搜再搜一下”“用盘搜搜”“云盘再找一下”等续问，而上一轮识别名是“片名 (年份)”或“片名（年份）”时，构造盘搜命令只使用片名，不把括号年份拼入搜索词。例如上一轮识别“黑夜告白 (2026)”，应 route “盘搜搜索 黑夜告白”，不要发“盘搜搜索 黑夜告白 2026”。aro_request.py route 已自动处理 `盘搜搜索 片名 (年份)` 的剥离。用户明确说“2026版”“只看2026版”“按2026搜”“带2026”时才保留年份。
```

## Search

```text
使用 agent-resource-officer skill，`更新检查 <片名>` / `查更新 <片名>` / `检查 <片名>` 已并回搜索语义。用户输入这些命令时，先直接调用 route 对应原话，不要先清空会话，不要先网页搜索，不要先走影巢候选。`检查115登录` 仍然是 115 登录检查，不是搜索命令。
搜索返回后必须原样保留插件 message 的 emoji 分区和条目行。不要改写成字段表，也不要把条目压缩成总结。
```
...
```text
使用 agent-resource-officer skill，执行”检查 大君夫人”或”检查大君夫人”时，先直接 route 原话，插件会将其重定向到搜索语义。不要先清空会话；但 `检查115登录` 仍然保持登录检查。
```

```text
使用 agent-resource-officer skill，执行”更新 大君夫人”时，先直接 route 原话，插件会将其重定向到搜索语义。如果返回里已经列出资源，不要再改写成”你要更新的是不是选项1”。
```

```text
使用 agent-resource-officer skill，处理普通”搜索/找片”时，不要先调用 `session-clear default`。只有用户明确要求”清空会话/重置会话”时，才允许先清会话。
```

```text
使用 agent-resource-officer skill，用户明确发出新的带片名入口命令，例如“搜索低智商犯罪”“搜索 低智商犯罪”“MP搜索 片名”“PT搜索 片名”“下载 片名”“转存 片名”时，必须先 route 原话。即使 route 返回 500、提示有活跃会话或发现旧候选列表，也不能自动 `pick 1` / `选择 1` / 提交下载或转存。只能重试同一原话、用同一原话换新 session，或把错误报告给用户。只有用户本轮明确说“选择 14 / 14详情 / 下载10 / 执行计划 / 刚才那个 / 上次的 / 原来的”这类旧会话续接意图，才允许恢复旧会话并 pick。
```
...
```text
使用 agent-resource-officer skill，执行”清空夸克默认转存目录”或”清空夸克默认目录”时，直接原样透传给 route。不要改写成 115 清理，不要先做搜索。
不要先 grep `aro_request.py` 或自己判断 helper 是否“内置支持”这个命令；这类清空命令本来就是通过 `route "<原话>"` 进入插件路由，不是 helper 的独立子命令。
```

```text
使用 agent-resource-officer skill，执行”清空115转存目录””清空115默认转存目录”或”清空115默认目录”时，直接原样透传给 route。不要改写成夸克清理，不要先做搜索。
不要先 grep `aro_request.py` 或自己判断 helper 是否“内置支持”这个命令；这类清空命令本来就是通过 `route "<原话>"` 进入插件路由，不是 helper 的独立子命令。
```

```text
使用 agent-resource-officer skill，返回盘搜/影巢的资源列表时，必须保留插件原始编号。不要把 `7/8/9/14` 这类编号改写成无编号段落，也不要只在总结里提编号。用户后续需要直接回复编号执行。
```
...
使用 agent-resource-officer skill，盘搜翻页时必须保留插件返回的“展示全局编号 11-20 / 21-30”这类提示，并按全局编号继续。不要把第二页重新改写成“1-10”，也不要把“回复 11”改写成“回复 1”。
```
...
```text
使用 agent-resource-officer skill，用户说“下载 蜘蛛侠”“转存 蜘蛛侠”“夸克转存 蜘蛛侠”“115转存 蜘蛛侠”这类写入命令时，必须保留原话交给 route。现在“下载”仍是有效主线；标题级“转存/115转存/夸克转存”会由插件直接返回“命令已取消”，并引导先做 `盘搜搜索` / `影巢搜索` / `PT搜索`。不要把这些命令改写成“智能执行 蜘蛛侠”，也不要跳过影片确认。
```
...
```text
使用 agent-resource-officer skill，在 PT 结果列表里，裸编号”1”、`下载1`，以及用户误发的”计划1/生成计划1”，都按直接下载当前第 1 条 PT 结果处理；”1详情”不是普通聊天入口。只有插件已经返回”PT 下载计划已生成”之后，用户再回复裸编号”1”或”执行计划”才是确认执行。不要把”下载1”当成旧计划确认，也不要把插件的”回复编号直接下载”改写成”查看详情或生成下载计划”。
```

```text
使用 agent-resource-officer skill，即使 PT 条目带 `risk_reasons` 或 `score_level=confirm`，只要插件顶层 `score_summary.decision` 给出 `preferred_requires_confirmation=false`、`command_policy=auto_continue` 或 `pt_direct_download=true`，就不得拦截为”确认下载吗？”。PT 的 `risk_reasons` 是提醒不是阻断，`hard_risk_reasons` 才是真正阻断依据。顶层 command summary 优先于嵌套评分标签，PT 列表的裸编号语义优先于通用 confirm 口径。
```

```text
使用 agent-resource-officer skill，用户说“校准影视技能”时，先定位 MoviePilot-Plugins 仓库并检查更新：执行 git fetch origin --prune；如果工作区干净，再执行 git pull --ff-only origin 当前分支。若当前 skill 是 WorkBuddy 这类 standalone 安装副本，拉取后还要从仓库 skill 目录同步当前 skill。若有本地未提交改动或无法快进，只报告原因并继续校准，不要覆盖本地改动。随后运行 python3 scripts/aro_request.py calibrate 或 route "校准影视技能"；无论 pull 是否跳过，都必须重新读取本地 SKILL.md 和 EXTERNAL_AGENTS.md，把返回的硬规则覆盖到当前会话，然后回复仓库更新/同步状态和“影视技能已校准。”。这个命令用于长线程、微信线程或会话压缩后重新校准资源流语义，避免把“下载”改成云盘转存、把“详情”改成执行。
```
...
```text
使用 agent-resource-officer skill，如果“影巢搜索”因为暂无结果而自动补查盘搜，或“盘搜搜索”里的影巢段没有展开，优先原样展示插件返回。不要把它改写成只剩“有新集了”“现在两边都有了”“最高分如下”这类摘要；必须保留原始编号、原始链接和下一步提示。可以在列表后追加智能建议，建议不限制长短。
```
...
用户明确说“MP搜索 / MP 搜索 / PT搜索 / PT 搜索 某片”时，必须保持原命令进入 route，不要改写为“搜索/盘搜搜索/影巢搜索/智能搜索”。
“推荐 / 热门推荐 / 智能发现 / 热门发现 / 资源决策 / 智能决策”这组高级入口已经下线。不要再主动建议用户使用它们，也不要把普通明确来源命令自动升级成智能决策。
用户只说”更新/查更新/检查 某片”时，先原样透传给 route，插件会将其重定向到搜索语义；不要先清空会话，不要先影巢候选，不要先网页搜索。
```
...
创建的 Skill 必须写入这些硬规则：
1. 资源流命令直接走 agent-resource-officer 的 route/pick，不要先走 MCP、tool_search、curl 或 raw API。当前主线包括：盘搜搜索、影巢搜索、MP搜索、PT搜索、下载、搜索、选择、详情、n、下一页和编号续选。`更新检查` / `查更新` / `检查` 已并回搜索语义。标题级”转存/115转存/夸克转存”仅用于返回插件的取消提示，不再是实际执行入口。
