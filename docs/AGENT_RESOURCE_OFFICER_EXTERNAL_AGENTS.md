...
同机时不要把 `ARO_BASE_URL` 填成本机局域网 IP，例如 `http://192.168.x.x:3000`。
尤其是 Docker / OrbStack 这类本机容器环境，外部智能体和 MoviePilot 明明在同一台机器，却走局域网 IP，可能出现 `assistant/route 500`、`selfcheck 正常但 搜索 失败` 这种绕路问题。
同机就直接用 `http://127.0.0.1:3000`，最稳。
