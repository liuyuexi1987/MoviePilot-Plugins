...
```text
AgentResourceOfficer-0.3.5.zip
...
| 盘搜搜索 | `盘搜 API 地址` |
| 影巢搜索 | `影巢 OpenAPI Key` |
| 115 转存 | `115 默认目录`；推荐搭配 `P115StrmHelper`，没有它也可以直接发 `115登录` 扫码 |
...
- `搜索 <片名>` 现在默认优先走 `MP/PT`；如果 `MP/PT` 已关闭，再按当前启用源回退。
- `刷新夸克Cookie` / `刷新影巢Cookie` 读取的是智能体所在电脑浏览器里的登录态，再写回 `MoviePilot`；跨机器部署时尤其要注意这点。
...
资源主线：`Agent影视助手 / AgentResourceOfficer 0.3.5`

当前已验证上游 MoviePilot：`v2.11.4`

当前 Skill helper：`agent-resource-officer 0.1.51`

当前 Release：https://github.com/liuyuexi1987/MoviePilot-Plugins/releases/tag/v0.3.5
